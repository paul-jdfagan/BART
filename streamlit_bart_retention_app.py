...
if uploaded is not None:
    ...
    for ci, c in enumerate(cohorts):
        base = 0.45 + 0.05 * ci   # different baseline per cohort
...
# ------------------------------
# Exploratory Plots (match notebook)
# ------------------------------
...
if not train_df.empty:
    heat_df = (
        train_df.assign(
            cohort_fmt=lambda d: d[cohort_col].dt.strftime("%Y-%m") if np.issubdtype(train_df[cohort_col].dtype, np.datetime64) else d[cohort_col].astype(str),
...
if not train_df.empty:
    ...
    sns.lineplot(
        x=period_col, y="retention", hue="cohort_fmt", palette="viridis_r", alpha=0.8, data=plot_df, ax=ax_l
    )
...
eps = 1e-6
train_retention = train_df["retention"].values
train_retention = np.clip(train_retention, eps, 1 - eps)
...
n_active_test = test_df[n_active_col].values.astype(int) if len(test_df) else np.array([], dtype=int)
...
ax_ppc = az.plot_ppc(data=posterior_predictive, kind="cumulative", observed_rug=True, random_seed=42)
ax_ppc.set(title="Posterior Predictive Check", xscale="log", xlabel="likelihood (n_active_users) - log scale")
...
p_train = az.extract(idata, var_names=["p"]).to_array().squeeze().values  # shape: (draws*chains, n_train)

# Test: update data and predict 'p' via posterior predictive
with model:
    pm.set_data({"x": X_test})
    if len(test_df):
        pp_test = pm.sample_posterior_predictive(idata, var_names=["p"], predictions=True, extend_inferencedata=False, progressbar=False)
        p_test = pp_test["p"]  # shape: (draws, chains, n_test) or (draws*chains, n_test)
    else:
